...
def launch_rlg_hydra(cfg: DictConfig):
    ...
    cfg.task.env.numEnvs = cfg.task.eval.headingStep* cfg.task.eval.swivelStep
    ...
    def evaluate(player):
        ...
        eval_params = (np.array(np.meshgrid(heading, swivel)).T).reshape(-1, 2)
        h_arr = eval_params[:, 0]
        s_arr = eval_params[:, 1]
        v = 0.2
        w = 1.0
        ...
        while not player.env.gym.query_viewer_has_closed(player.env.viewer):
            if has_masks:
